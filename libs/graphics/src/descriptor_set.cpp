...
DescriptorSet::DescriptorSet(DescriptorSet&& rhs) noexcept : m_storage(std::exchange(rhs.m_storage, Storage())), m_device(rhs.m_device) {
}
...
void DescriptorSet::updateBuffers(u32 binding, Span<CView<Buffer>> buffers, std::size_t size, vk::DescriptorType type) {
	...
	if (stale(buffers, bind.buffers, [](CView<Buffer> lhs, CView<Buffer> rhs) { return lhs->buffer == rhs->buffer; })) {
...
void DescriptorSet::update(vk::WriteDescriptorSet write) {
	m_device.get().m_device.updateDescriptorSets(write, {});
}
