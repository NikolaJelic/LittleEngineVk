...
bool init(LibLogger& logger) {
	g_log = &logger;
	glfwSetErrorCallback([](s32 code, char const* szDesc) { g_log->log(lvl::error, 1, "[{}] GLFW Error! [{}]: {}", s_name, code, szDesc); });
	if (glfwInit() != GLFW_TRUE) {
		g_log->log(lvl::error, 2, "[{}] Could not initialise GLFW!", s_name);
		return false;
	} else if (glfwVulkanSupported() != GLFW_TRUE) {
		g_log->log(lvl::error, 2, "[{}] Vulkan not supported!", s_name);
		return false;
	} else {
		g_log->log(lvl::info, 1, "[{}] GLFW initialised successfully", s_name);
	}
...
void deinit() {
	...
	glfwTerminate();
	g_state = {};
	g_log->log(lvl::info, 1, "[{}] GLFW terminated", s_name);
}
} // namespace
...
void onFocus(GLFWwindow* pGLFWwindow, s32 entered) {
	if (g_state.bInit && g_state.pWindow == pGLFWwindow) {
		...
		g_log->log(lvl::info, 1, "[{}] Window focus {}", s_name, (entered != 0) ? "gained" : "lost");
	}
}
...
void onWindowResize(GLFWwindow* pGLFWwindow, s32 width, s32 height) {
	if (g_state.bInit && g_state.pWindow == pGLFWwindow) {
		...
		g_state.events.m_events.push_back(event);
		g_log->log(lvl::info, 1, "[{}] Window resized: [{}x{}]", s_name, width, height);
	}
}
...
void onFramebufferResize(GLFWwindow* pGLFWwindow, s32 width, s32 height) {
	if (g_state.bInit && g_state.pWindow == pGLFWwindow) {
		...
		g_state.events.m_events.push_back(event);
		g_log->log(lvl::info, 1, "[{}] Framebuffer resized: [{}x{}]", s_name, width, height);
	}
}
...
void onIconify(GLFWwindow* pWindow, int iconified) {
	if (g_state.bInit && g_state.pWindow == pWindow) {
		...
		g_state.events.m_events.push_back(event);
		g_log->log(lvl::info, 1, "[{}] Window {}", s_name, iconified != 0 ? "suspended" : "resumed");
	}
}
...
void onClose(GLFWwindow* pGLFWwindow) {
	if (g_state.bInit && g_state.pWindow == pGLFWwindow) {
		...
		g_state.events.m_events.push_back(event);
		g_log->log(lvl::info, 1, "[{}] Window closed", s_name);
	}
}
...
DesktopInstance::DesktopInstance(CreateInfo const& info) : Instance(true) {
	...
	if (screenCount < 1) {
		m_log.log(lvl::error, 2, "[{}] Failed to detect screens!", s_name);
		throw std::runtime_error("Failed to create Window");
	}
	GLFWvidmode const* mode = glfwGetVideoMode(ppScreens[0]);
	if (!mode) {
		m_log.log(lvl::error, 2, "[{}] Failed to detect video mode!", s_name);
		throw std::runtime_error("Failed to create Window");
	}
	...
	case Style::eDecoratedWindow: {
		if (mode->width < width || mode->height < height) {
			m_log.log(lvl::error, 2, "[{}] Window size [{}x{}] too large for default screen! [{}x{}]", s_name, width, height, mode->width, mode->height);
			throw std::runtime_error("Failed to create Window");
		}
		pTarget = nullptr;
		break;
